...
class BlackListFilters2(BASE):
    __tablename__ = "blacklist2"
...
class BlacklistSettings2(BASE):
    __tablename__ = "blacklist_settings2"
    chat_id = Column(String(14), primary_key=True)
...
def add_to_blacklist2(chat_id, trigger):
...
def rm_from_blacklist2(chat_id, trigger):
    with BLACKLIST_FILTER_INSERTION_LOCK:
...
def get_chat_blacklist2(chat_id):
    return CHAT_BLACKLISTS.get(str(chat_id), set())


def num_blacklist_filters2():
    try:
...
def num_blacklist_chat_filters2(chat_id):
    try:
...
def num_blacklist_filter_chats2():
    try:
...
def set_blacklist_strength2(chat_id, blacklist_type, value):
    # for blacklist_type
    # 0 = nothing
    # 1 = delete
    # 2 = warn
...
def get_blacklist_setting2(chat_id):
    try:
